...
export const updateField = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { name, field_type, price_per_hour, description, image, is_active } =
      req.body;
    ...
    // If no new images uploaded, don't update the images field (keep existing)
    ...
    const field = await prisma.field.update({
      ...
      data: {
        ...
        description,
        ...(imageData !== undefined && { images: imageData }),
        ...(isActiveValue !== undefined && { is_active: isActiveValue }),
      },
    });
